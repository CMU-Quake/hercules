...
static int32_t oct_expand(oct_t *leafoct, tree_t *tree,
                          toexpand_t *toexpand, setrec_t *setrec, int setrecMode);
...
oct_expand(oct_t *leafoct, tree_t *tree, toexpand_t *toexpand,
           setrec_t *setrec, int setrecMode)
{
    int8_t which;
    const void *data;
    ...
    if (isOverlapped ||
        ...
        for (which = 0; which < 8; which++) {
            ...
            if (child != NULL) {
                ...
                setrec((octant_t *)child, tree->ticksize, data, setrecMode);

                /* This child intersects the domain of interest */
                if (oct_expand(child, tree, toexpand, setrec, setrecMode) != 0)
                    return -1;
            }
        }
    }
 ...
 */
extern int32_t
octor_refinetree(octree_t *octree, toexpand_t *toexpand, setrec_t *setrec, int setrecMode)
{
    tree_t *tree = (tree_t *)octree;
    oct_t *oct;
    ...
    while ((oct != NULL) && (oct->where == LOCAL)) {
        if (oct_expand(oct, tree, toexpand, setrec, setrecMode) != 0) {
            fprintf(stderr,
 ...
 */
extern int32_t
octor_balancetree(octree_t *octree, setrec_t *setrec, int setrecMode, int theStepMeshingFactor)
{
